...
function alertar_validar_codigo_activo(){
  ...
  var alerta ='<div class="toast" id="activo_alerta">'
  +'<div class="toast-header">'
  ...
  document.getElementById('cerrar_alerta_activo').addEventListener("click", function(){
  document.getElementById("boton_enviar_primera").disabled = false;
});


  setTimeout(function(){ $('#activo_alerta').remove(); }, 2000);
  setTimeout(function(){ $('#boton_enviar_primera').prop( "disabled", false ); }, 2000);
...
function animacion_enviado_codigo_primero(){
  ...
  var boton_falso = '<button id = "boton_enviar_falso" type="submit" class="btn btn-primary disabled boton_success"> <span class="spinner-border spinner-border-sm"></span> enviando correo</button>';
  $("#enviar_correo_boton").append(boton_falso);
   
 }

 function restaurar_icono_enviar(){
  var boton_real = '<button  id = "boton_enviar_primera" type="submit" class="btn btn-primary boton_success"><i id="icono_reenviar"class="fas fa-share"></i> enviar correo</button>'
