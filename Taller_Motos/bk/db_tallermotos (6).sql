...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 26-07-2020 a las 00:39:28
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_compra` (IN `id` INT)  begin
        ...
        concat(m.nombre,' ',mm.nombre_marca,' ',if(m.presentacion>0,m.presentacion,''),' ',m.unidad) as nombre_material, m.id_material as material from compras_materiales c_m
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_detalle_compra` (IN `id` INT)  begin
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_detalle_reparacion` (IN `id` INT)  begin
	select m.nombre as nombre, m.unidad as medida,m.cantidad_inicial as cantidad,m.cantidad_inicial*m.presentacion as total 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_detalle_venta` (IN `id` INT)  begin
		select  v.id_venta as id, v_m.precio*v_m.cantidad as saldo 
        , v_m.precio as precio, v_m.cantidad as cantidad, 
        concat(m.nombre,' ',mm.nombre_marca,' ',if(m.presentacion>0,m.presentacion,''),' ',m.unidad) as nombre_material, m.id_material as material  
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_inventario` ()  begin
		...
		,if(m.presentacion>0,m.presentacion,'') as monto, mm.nombre_marca as marca, if( m.unidad='unid',m.cantidad_inicial+coalesce(compra,0)-coalesce(venta,0)-coalesce(reparacion,0),(m.cantidad_inicial+coalesce(compra*m.presentacion,0)-coalesce(venta*m.presentacion,0)-coalesce(reparacion,0))/m.presentacion) as saldo 
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_material` (IN `id` INT)  begin
		select * from materiales m 
		inner join marcas_materiales mm 
		inner join categorias_material cm where
		mm.id_marca_material= m.id_marca_material and  m.id_categoria=cm.id_categoria and m.id_material=id;
	 end$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_reparacion_moto` (IN `placa_moto` INT)  begin
    ...
    m.numero_placa as placa, r.precio as monto, m.id_moto as id_moto, r.kilometraje_entrada as kilometraje,r.estado as estado from  reparaciones r 
    inner join motos m on r.id_moto=m.id_moto
    inner join clientes on clientes.id_cliente = m.id_cliente
    inner join marcas_motos ma on ma.id_marca_moto = m.id_marca_moto inner join
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `get_venta` (IN `id` INT)  begin
        ...
        , v_m.precio as precio, v_m.cantidad as cantidad, 
        concat(m.nombre,' ',mm.nombre_marca,' ',if(m.presentacion>0,m.presentacion,''),' ',m.unidad) as nombre_material, m.id_material as material,u.nombre_completo as usuario  
        from ventas_materiales v_m
            inner join ventas v on v_m.id_venta=v.id_venta
			inner join materiales m on m.id_material = v_m.id_material
            inner join marcas_materiales mm on mm.id_marca_material=m.id_marca_material
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_compra` (IN `id` INT, IN `proveedor` INT, IN `factura` INT, IN `fecha` DATE, IN `usuario` INT)  begin
		update compras c set c.id_proveedor=proveedor, c.factura=factura, c.fecha=fecha,c.id_usuario=usuario where c.id_compra=id;
	end$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `update_venta` (IN `id` INT, IN `cliente` INT(50), IN `fecha` DATE, IN `usuario` INT)  begin
		update ventas v set v.id_cliente=cliente,v.fecha=fecha,v.id_usuario=usuario where v.id_venta=id;
	end$$
...
(13, '4121414', 'Pablo coelo', 'p@gmail.com', '17', '123', 1),
(14, '20393818', 'Mauricio', 'M@gmail.com', '214151351', '123', 1);
...
(1, 'Migthy motors', 'assets/logo/159portada.jpg', 'mannolo@gmail.com', 'B° San Rafael', '20393818', '83595176', '1234');
...
(1, 'Mannol', '22222222', 'mannolo@gmail.com', '38041074', 1),
(2, 'Firestone', '27652425', 'Fire@gmail.com', '415521531', 1);
...
(18, 'Pablo coelo', 'p@gmail.com', 'cliente', '123', 1, '', ''),
(19, 'Mauricio', 'M@gmail.com', 'cliente', '123', 1, '', '');
